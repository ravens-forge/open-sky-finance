...
- Categories page with Expenses and Income tabs: your groups, the categories inside them, and reordering by dragging or moving a row up and down. A category has a name, a group, an icon, a colour of its own or its group's, and can be hidden from the pickers; a group has a name, a type that locks once something uses it, and can hide its categories with it. Deleting asks where its transactions and reminders go — to its group, to another category or nowhere — and a group has to be emptied first. Pickers search without worrying about accents and let you choose a whole group.
